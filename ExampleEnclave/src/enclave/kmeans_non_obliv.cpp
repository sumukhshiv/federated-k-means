...
double calc_distance_nonobliv(int dim, double *p1, double *p2)
  {
...
void calc_all_distances_nonobliv(int dim, int n, int k, double *X, double *centroid, double *distance_output)
  {
      ...
      for (int jj = 0; jj < k; jj++) // for each cluster
        {
         // calculate distance between point and cluster centroid
          distance_output[ii*k + jj] = calc_distance_nonobliv(dim, &X[ii*dim], &centroid[jj*dim]);
        }
  }
  
double calc_total_distance_nonobliv(int dim, int n, int k, double *X, double *centroids, int *cluster_assignment_index)
    ...
    for (int ii = 0; ii < n; ii++)
      {
       ...
       // sum distance
        if (active_cluster != -1)
          tot_D += calc_distance_nonobliv(dim, &X[ii*dim], &centroids[active_cluster*dim]);
      }
      
    return tot_D;
  }

void choose_all_clusters_from_distances_nonobliv(int dim, int n, int k, double *distance_array, int *cluster_assignment_index)
  {
   // for each point
    for (int ii = 0; ii < n; ii++)
      {
...
void calc_cluster_centroids_nonobliv(int dim, int n, int k, double *X, int *cluster_assignment_index, double *new_cluster_centroid)
  {
...
void get_cluster_member_count_nonobliv(int n, int k, int *cluster_assignment_index, int *cluster_member_count)
  {
...
void update_delta_score_table_nonobliv(int dim, int n, int k, double *X, int *cluster_assignment_cur, double *cluster_centroid, int *cluster_member_count, double *point_move_score_table, int cc)
  {
...
void  perform_move_nonobliv(int dim, int n, int k, double *X, int *cluster_assignment, double *cluster_centroid, int *cluster_member_count, int move_point, int move_target_cluster)
  {
...
void cluster_diag_nonobliv(int dim, int n, int k, double *X, int *cluster_assignment_index, double *cluster_centroid)
  {
    int cluster_member_count[MAX_CLUSTERS];
    
    get_cluster_member_count_nonobliv(n, k, cluster_assignment_index, cluster_member_count);
     
    ocall_print("  Final clusters \n");
    for (int ii = 0; ii < k; ii++) {
...
void copy_assignment_array_nonobliv(int n, int *src, int *tgt)
  {
    for (int ii = 0; ii < n; ii++)
      tgt[ii] = src[ii];
  }
  
int assignment_change_count_nonobliv(int n, int a[], int b[])
  {
    int change_count = 0;
...
void kmeans_nonobliv(
           ...
           )
  {
   ...
   // initial setup  
    calc_all_distances_nonobliv(dim, n, k, X, cluster_centroid, dist);
    choose_all_clusters_from_distances_nonobliv(dim, n, k, dist, cluster_assignment_cur);
    copy_assignment_array_nonobliv(n, cluster_assignment_cur, cluster_assignment_prev);
...
//        cluster_diag(dim, n, k, X, cluster_assignment_cur, cluster_centroid);
        ...
        // update cluster centroids
         calc_cluster_centroids_nonobliv(dim, n, k, X, cluster_assignment_cur, cluster_centroid);

        // deal with empty clusters
        // XXXXXXXXXXXXXX

        // see if we've failed to improve
         double totD = calc_total_distance_nonobliv(dim, n, k, X, cluster_centroid, cluster_assignment_cur);
         if (totD > prev_totD)
          // failed to improve - currently solution worse than previous
           {
            // restore old assignments
             copy_assignment_array_nonobliv(n, cluster_assignment_prev, cluster_assignment_cur);
             
            // recalc centroids
             calc_cluster_centroids_nonobliv(dim, n, k, X, cluster_assignment_cur, cluster_centroid);
             char* hello_world = (char*)malloc(150 * sizeof(char));
             snprintf(hello_world, 150, "  negative progress made on this step - iteration completed (%.2f) \n", totD - prev_totD);
             ocall_print(hello_world);             
        ...
        // move all points to nearest cluster
         ...
         int change_count = assignment_change_count_nonobliv(n, cluster_assignment_cur, cluster_assignment_prev);
         char* hello_world = (char*)malloc(150 * sizeof(char));
         snprintf(hello_world, 150, "%3d   %u   %9d  %16.2f %17.2f\n", batch_iteration, 1, change_count, totD, totD - prev_totD);
         ocall_print(hello_world);    
        //  fflush(stdout);
        ...
        // done with this phase if nothing has changed
         if (change_count == 0)
           {
             ocall_print("  no change made on this step - iteration completed \n");
             break;
           }
...
cluster_diag_nonobliv(dim, n, k, X, cluster_assignment_cur, cluster_centroid);
...
//    printf("iterations: %3d %3d \n", batch_iteration, online_iteration);
   ...
   // write to output array
    copy_assignment_array_nonobliv(n, cluster_assignment_cur, cluster_assignment_final);    
