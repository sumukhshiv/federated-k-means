...
int main() {
  ...
  TEST(SE<bool, H> b = pl == pl; assert(se_to_primitive(b) == true)); // CHECK: TEST
  // CHECK-NEXT: SE<O<ListIt, List>, H>, SE<O<ListIt, List>, H>: operator==
  // CHECK: END TEST

  TEST(SE<bool, H> b = pl != pl; assert(se_to_primitive(b) == false)); // CHECK: TEST
  // CHECK-NEXT: SE<O<ListIt, List>, H>, SE<O<ListIt, List>, H>: operator!=
  // CHECK: END TEST
  ...
  TEST(SE<bool, H> b = pv == pv; assert(se_to_primitive(b) == true)); // CHECK: TEST
  // CHECK-NEXT: SE<O<VectorIt, Vector>, H>, SE<O<VectorIt, Vector>, H>: operator==
  // CHECK: END TEST

  TEST(SE<bool, H> b = pv != pv; assert(se_to_primitive(b) == false)); // CHECK: TEST
  // CHECK-NEXT: SE<O<VectorIt, Vector>, H>, SE<O<VectorIt, Vector>, H>: operator!=
  // CHECK: END TEST
